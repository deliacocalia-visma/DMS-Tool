...
def main():
    ...
    # Loop through existing tasks and delete tasks that are no longer needed
    for task in existing_tasks:
        task_identifier = task['ReplicationTaskIdentifier']
        tags = client.list_tags_for_resource(ResourceArn=task['ReplicationTaskArn'])['TagList']
        if any(tag['Key'] == 'Generated by' and tag['Value'] == 'GitHub DMS Tool' for tag in tags):
            if task_identifier not in [f"{table['name']}-{table['schema_name']}-{table['target_schema_name']}" for table in table_actions]:
                try:
                    ...
                    print(f"Deleted DMS Replication Task {task_identifier}")
                except Exception as e:
                    print(f"An error occurred while deleting DMS Replication Task {task_identifier}: {e}")
