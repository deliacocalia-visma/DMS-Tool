...
def get_tables(filename='table_names.yml'):
    ...
    for key in file_contents:
        ...
        table = {'name': key, 'schema_name': schema_name, 'target_schema_name': target_schema_name}
...
def main():
    ...
    for task in existing_tasks:
        if task['ReplicationTaskIdentifier'] not in task_identifiers and False:
            # Check if the task was generated by the GitHub DMS Tool
            if 'Tags' in task:
                tags = task['Tags']
                if any(tag['Key'] == 'Generated by' and tag['Value'] == 'GitHub DMS Tool' for tag in tags):
                    try:
                        client.delete_replication_task(ReplicationTaskArn=task['ReplicationTaskArn'])
                        print(f"Deleted DMS Replication Task {task['ReplicationTaskIdentifier']}")
                    except Exception as e:
                        print(f"An error occurred while deleting DMS Replication Task {task['ReplicationTaskIdentifier']}: {e}")
